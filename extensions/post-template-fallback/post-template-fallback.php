            ...
            var unsubscribe = window.wp.data.subscribe(function() {
                ...
                function checkBlocks(blockList) {
                    blockList.forEach(function(block) {
                        // Handle query blocks
                        if (block.name === 'core/query' && block.attributes.query) {
                            // Store valid query states
                            if (block.attributes.query.postType && block.attributes.query.postType !== 'post') {
                                lastQueryState[block.clientId] = JSON.parse(JSON.stringify(block.attributes.query));
                            }
                            
                            // If query was reset to default 'post', restore it
                            if (block.attributes.query.postType === 'post' && 
                                lastQueryState[block.clientId] && 
                                lastQueryState[block.clientId].postType !== 'post') {
                                
                                console.log('Emergency restoration of query block:', lastQueryState[block.clientId]);
                                
                                window.wp.data.dispatch('core/block-editor').updateBlockAttributes(
                                    block.clientId, 
                                    { query: lastQueryState[block.clientId] }
                                );
                            }
                        }
                        
                        // Handle cover blocks in query context
                        if (block.name === 'core/cover' && !coverBlocksFixed[block.clientId]) {
                            ...
                            if (queryBlock && queryBlock.attributes.query && queryBlock.attributes.query.postType !== 'post') {
                                console.log('Fixing cover block for post type:', queryBlock.attributes.query.postType);
                                ...
                                if (block.attributes.useFeaturedImage) {
                                    ...
                                    needsUpdate = true;
                                    console.log('Disabling useFeaturedImage on cover block');
                                }
                                
                                if (block.attributes.linkToPost) {
                                    newAttributes.linkToPost = false;
                                    needsUpdate = true;
                                    console.log('Disabling linkToPost on cover block');
                                }
